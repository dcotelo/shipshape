...
4. Arguments: `audit` or `scaffold` select the mode intent; `dry-run` selects the read-only mode, whose contract is in `../../AGENTS.md`; a profile (`public-oss` | `internal`) may follow. AGENTS.md still requires detecting the mode and confirming the profile with the user.
